        ...
        <div className="container mx-auto px-6">
          ...
          <div className="max-w-5xl mx-auto glass-card rounded-[40px] border border-slate-200 overflow-hidden bg-white/40">
            <div className="grid grid-cols-3 p-8 md:p-12 border-b border-slate-200 bg-slate-50/50">
              <div className="text-xs md:text-sm font-black uppercase tracking-widest text-slate-400">Feature</div>
              <div className="text-xs md:text-sm font-black uppercase tracking-widest text-slate-400">Manual Support</div>
              <div className="text-xs md:text-sm font-black uppercase tracking-widest text-whatsapp-green">Vixingo Agent</div>
            </div>
            
            <div className="divide-y divide-slate-200">
              {COMPARISON.map((row, i) => (
                <div key={i} className="grid grid-cols-3 p-8 md:p-12 items-center hover:bg-slate-50/50 transition-colors group">
                  <div className="text-sm md:text-lg font-bold text-slate-900 group-hover:translate-x-1 transition-transform">{row.feature}</div>
      ...
      <footer className="py-20 border-t border-slate-200 relative z-10">
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-20">
            <div className="md:col-span-2">
              <div className="flex items-center gap-2 mb-8">
                <div className="w-10 h-10 rounded-xl bg-whatsapp-green flex items-center justify-center text-white font-black text-xl">V</div>
                <span className="text-2xl font-black tracking-tighter text-slate-900">Vixingo</span>
              </div>
              ...
              <div className="flex gap-4">
                {[Twitter, Linkedin, Instagram].map((Icon, i) => (
                  <a key={i} href="#" className="w-12 h-12 rounded-2xl glass-card flex items-center justify-center text-slate-400 hover:text-whatsapp-green hover:border-whatsapp-green/20 transition-all bg-white/40 border-slate-200">
                    <Icon size={20} />
                  </a>
                ))}
              </div>
            ...
            <div>
              <h4 className="text-slate-900 font-black mb-8 uppercase tracking-widest text-xs">Product</h4>
              <ul className="space-y-4">
                {['Features', 'Integrations', 'Pricing', 'API Docs'].map((item) => (
                  <li key={item}><a href="#" className="text-slate-500 hover:text-slate-900 transition-colors font-body">{item}</a></li>
                ))}
              </ul>
            </div>

            <div>
              <h4 className="text-slate-900 font-black mb-8 uppercase tracking-widest text-xs">Company</h4>
              <ul className="space-y-4">
          ...
          <div className="pt-8 border-t border-slate-200 flex flex-col md:row items-center justify-between gap-6">
            <p className="text-xs text-slate-400 font-body">© 2026 Vixingo. All rights reserved.</p>
            <p className="text-[10px] text-slate-300 font-body uppercase tracking-[0.2em]">WhatsApp is a trademark of Meta Platforms, Inc.</p>
          </div>
        </div>
      </footer>
    </>
