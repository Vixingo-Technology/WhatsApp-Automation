...
const PROBLEMS = [
  "Slow Responses", "Human Errors", "Lost Leads", "High Costs",
  "Zero Automation", "No Analytics", "Manual Booking", "Angry Customers",
  "Missed Revenue", "Hard to Scale", "Inefficient", "No Follow-ups",
  "Wasted Spend", "Messy Data", "Manual Entry", "Support Fatigue"
];
...
const ProblemBox = ({ word, index, scrollYProgress }: ProblemBoxProps) => {
  ...
  const { angle, distance, rotateZ } = useMemo(() => {
    ...
    const distance = 25 + Math.random() * 25; // How far from center it moves
    return {
      angle,
      distance,
      rotateZ: (Math.random() - 0.5) * 20, // Slight tilt for variety
    };
  }, [index]);

  // Tunnel animation logic:
  // Starts at center (0,0, -1000), moves towards camera (0,0, 500) and outwards
  const opacity = useTransform(scrollYProgress, [0, 0.1, 0.5, 0.8], [0, 1, 1, 0]);
  ...
  const translateX = useTransform(scrollYProgress, [0.1, 0.7], ["0vw", `${xPos}vw`]);
  const translateY = useTransform(scrollYProgress, [0.1, 0.7], ["0vh", `${yPos}vh`]);
  
  // Z moves from deep in the tunnel towards the user
  const translateZ = useTransform(scrollYProgress, [0, 0.8], [-1500, 1000]);
  
  const scale = useTransform(scrollYProgress, [0, 0.2, 0.6, 0.8], [0.2, 1, 1.2, 0.5]);
...
export function ProblemSection({ containerRef }: { containerRef?: React.RefObject<HTMLDivElement | null> }) {
  ...
  const charIndex = useTransform(smoothProgress, [0.65, 0.75, 1.0], [text1.length, 0, text2.length]);
  ...
  const cursorColor = useTransform(smoothProgress, (v) => v > 0.75 ? '#25D366' : '#EF4444');
  const part2Style = useTransform(smoothProgress, (v) => {
    if (v < 0.75) return "text-red-500 drop-shadow-[0_0_15px_rgba(239,68,68,0.8)]"; // Glowing red
    return "text-whatsapp-green"; // WhatsApp Green
  });

  return (
    <section ref={sectionRef} id="problem" className="relative h-[600vh] bg-transparent">
      <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden perspective-2000 bg-transparent">
        ...
        <div className="relative z-20 text-center px-6 max-w-7xl">
          <h2 className="text-4xl md:text-7xl lg:text-8xl font-black leading-[1.1] tracking-tighter">
            <motion.span className="text-slate-800" children={textPart1} />
            <motion.span
        ...
        >
          <span className="text-slate-600 text-[10px] font-bold uppercase tracking-[0.4em]">Scroll Down</span>
